...
class RRWPLinearEdgeEncoder(nn.Module):
    ...
    def forward(self, batch: object) -> object:
        ...
        Sparse originals are stored on ``batch.edge_index_mp`` /
        ``batch.edge_attr_mp`` before padding so hybrid MP heads keep the
        true graph topology when attention uses a full-graph edge set.
        """
        # Preserve sparse graph edges for message-passing heads.
        batch.edge_index_mp = batch.edge_index
        batch.edge_attr_mp = batch.edge_attr
