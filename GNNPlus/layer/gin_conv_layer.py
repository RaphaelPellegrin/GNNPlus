"""GIN convolution layer for ``custom_gnn`` (no edge features)."""
...
class GINConvLayer(nn.Module):
    """Batch-style GIN layer with optional residual and FFN (GNN+ style)."""
    ...
    def __init__(
        ...
        """Build a GINConv block at width ``dim_in`` → ``dim_out``.
    ...
    def forward(self, batch: object) -> object:
        """Update ``batch.x`` via GINConv (+ optional residual / FFN)."""
        x_in = batch.x
