...
Example:
    python scripts/api_wanndb_query/aggregate_tu_depth_dh_h_mutag.py
    python scripts/api_wanndb_query/aggregate_tu_depth_dh_h_mutag.py --from-csv
...
def plot_heatmaps(df: pd.DataFrame, out: Path, ds_label: str, hs: list[int]) -> None:
    ...
    for H in hs:
        ...
        legend_handles = [
            ...
            Line2D([0], [0], linestyle="none", marker="", label="g / u = gated / ungated mean±std (%)"),
            Line2D([0], [0], linestyle="none", marker="", label="* p<0.05   ** p<0.01   *** p<0.001"),
            Line2D([0], [0], linestyle="none", marker="", label="n.s.  paired t-test not significant (p≥0.05)"),
        ]
        ax.legend(
...
def fetch_from_wandb(ds: str, hs: list[int]) -> pd.DataFrame:
    """Query W&B and return best-LR summary table."""
...
def main() -> None:
    """CLI: fetch or reuse CSV, then write annotated heatmaps."""
    ...
    parser.add_argument(
        "--dataset",
        default="mutag",
        help="W&B dataset slug (default: mutag).",
    )
    parser.add_argument(
        "--H",
        type=int,
    ...
    ds = args.dataset
    ds_label = ds.upper().replace("_", "-") if ds != "mutag" else "MUTAG"
    if ds == "mutag":
        ds_label = "MUTAG"
    ...
    if args.from_csv:
        ...
        df = df[df["H"].isin(hs)].copy()
    else:
        df = fetch_from_wandb(ds, hs)
        # Merge with existing CSV rows for other H if present.
        if csv_path.is_file():
