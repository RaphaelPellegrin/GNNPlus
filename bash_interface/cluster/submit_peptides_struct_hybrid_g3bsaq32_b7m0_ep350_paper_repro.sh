#!/usr/bin/env bash
# Submit peptides-struct paper repro: g3bsaq32 b7_m0 × 10 seeds, ep=350 (extended training).
#
...
TIME="${PEPTIDES_STRUCT_PAPER_TIME:-336:00:00}"
WANDB_GROUP="paper_bestmodel_v2_peptides_struct_g3bsaq32_b7m0_ep350"

job_id="$(
    sbatch --parsable \
        --job-name=peptides_struct_g3bsaq32_ep350 \
        --array="${ARRAY_SPEC}%${PARALLEL}" \
        --partition=mweber_gpu \
        --mem="${MEM}" \
        --time="${TIME}" \
        --gpus=1 \
        --output="logs_gnnplus/peptides_struct_g3bsaq32_ep350_%A_%a.log" \
        --export=ALL,ENV_NAME=gnnplus,PAPER_NUM_SEEDS="${NUM_SEEDS}",PAPER_MAX_EPOCH=350,PAPER_WANDB_GROUP="${WANDB_GROUP}",GNNPLUS_DATASET_DIR="${GNNPLUS_DATASET_DIR:-}" \
        bash_interface/cluster/run_peptides_struct_hybrid_g3bsaq32_b7m0_paper_repro.sh
)"

echo ""
echo "=== peptides-struct g3bsaq32 b7_m0 paper repro (ep350) submitted ==="
