#!/usr/bin/env bash
# =============================================================================
# SiGMA paper Table 6 (code: paper_T5) — attention-gated / MP-ungated ablation
#
# Same LRGB anchors as Table 5 SiGMA, but:
#   gnn.hybrid.mp_gate none
# while keeping yaml ``gate`` (elementwise peptides / headwise VOC+COCO).
#
# 4 datasets × 1 variant × 5 seeds = 20 tasks.
#
# W&B group:  paper_T5_<dataset>_SiGMA_attn_gate
...
#SBATCH --job-name=sigma_T5_attn_gate
#SBATCH --ntasks=1
...
wandb_group_prefix="${PAPER_T5_ATTN_GATE_WANDB_PREFIX:-paper_T5}"
...
log_message "Table5 attn_gate task ${task_id}/${num_tasks}: ds=${ds_tag} variant=${variant} seed=${seed} source=${source_run} cfg=${cfg}"
...
if [ -n "${PAPER_T5_ATTN_GATE_MAX_EPOCH:-}" ]; then
    extra_args+=(optim.max_epoch "${PAPER_T5_ATTN_GATE_MAX_EPOCH}")
    log_message "max_epoch override: ${PAPER_T5_ATTN_GATE_MAX_EPOCH}"
fi
