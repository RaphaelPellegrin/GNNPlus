...
case "${CAMPAIGN}" in
    ...
    grid_select)
        ...
        ;;
    grid_eval|sigma_grid_eval|sigma_grid_eval_fixed8|sigma_grid_eval_full64|sigma_grid_eval_native_fair|sigma_grid_eval_native_fair_v2|sigma_grid_eval_native_fair_v2_l4|sigma_grid_eval_native_fair_v2_joint|sigma_grid_eval_native_fair_v2_reddit_reg_joint|sigma_grid_eval_proteins_reg_joint|sigma_grid_eval_anchor_boost|sigma_grid_eval_a1g2_micro|sigma_grid_eval_a1g2_nci1_micro|sigma_grid_eval_anchor_refine|sigma_grid_eval_nci1_refine|sigma_grid_eval_a0g_pnr|sigma_grid_eval_tiny_pnr|sigma_grid_eval_specialist_tiny|sigma_grid_eval_fixed8_ungated)
        ...
        if [ "${CAMPAIGN}" = "sigma_grid_eval_anchor_boost" ] \
            || [ "${CAMPAIGN}" = "sigma_grid_eval_a1g2_micro" ]; then
            # PROTEINS + REDDIT only (2 × folds × seeds).
            NUM_TASKS="${TU_ERRICA_NUM_TASKS:-$((2 * NUM_FOLDS * NUM_SEEDS))}"
        fi
        if [ "${CAMPAIGN}" = "sigma_grid_eval_a0g_pnr" ] \
            ...
            || [ "${CAMPAIGN}" = "sigma_grid_eval_native_fair_v2_joint" ]; then
            # PROTEINS + NCI1 + REDDIT (3 × folds × seeds).
            NUM_TASKS="${TU_ERRICA_NUM_TASKS:-$((3 * NUM_FOLDS * NUM_SEEDS))}"
        fi
        if [ "${CAMPAIGN}" = "sigma_grid_eval_a1g2_nci1_micro" ] \
            ...
            || [ "${CAMPAIGN}" = "sigma_grid_eval_proteins_reg_joint" ]; then
