#!/usr/bin/env bash
# =============================================================================
# SiGMA paper Table 6 (PascalVOC-SP) — homogeneous MP ± gates (fresh 5-seed runs).
#
# 2 variants × 5 seeds = 10 tasks (default).
#
# Variants (W&B group suffix — aligned with 1-MP Homog_* naming):
#   0  Homog_MP          — GATEDGCN,GATEDGCN + headwise gate (= SiGMA architecture)
#   1  Homog_MP_ungated  — GATEDGCN,GATEDGCN + gnn.hybrid.gate=none
#
...
#SBATCH --job-name=sigma_T6_voc_homog
#SBATCH --ntasks=1
...
case "${variant_idx}" in
    0)
        variant="Homog_MP"
        # Anchor as-is: GATEDGCN,GATEDGCN + headwise gate (same arch as SiGMA).
        ;;
    1)
...
wandb_group_prefix="${PAPER_T6_VOC_HOMOG_WANDB_PREFIX:-paper_T6}"
...
log_message "Table6 VOC Homog task ${task_id}/${num_tasks}: variant=${variant} seed=${seed} source=${source_run} cfg=${cfg}"
