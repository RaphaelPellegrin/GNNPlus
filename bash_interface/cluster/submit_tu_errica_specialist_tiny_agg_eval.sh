...
if [ -n "${DEPENDENCY_JOBID}" ]; then
  ...
  echo "[specialist_tiny agg_eval] no dependency (run immediately)"
fi
...
JOBID="$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/run_tu_errica_specialist_tiny_agg_eval.sh")"

echo "Submitted specialist_tiny agg→eval JOBID=${JOBID}"
if [ -n "${DEPENDENCY_JOBID}" ]; then
