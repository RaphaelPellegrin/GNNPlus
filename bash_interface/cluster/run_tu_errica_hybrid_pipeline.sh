...
#
# native_fair_v2_reddit_reg (REDDIT lr×dropout refine; UNION L12; gpu_h200 %15):
#   bash bash_interface/cluster/run_tu_errica_hybrid_pipeline.sh generate_sigma_grids_native_fair_v2_reddit_reg
...
#
# proteins_reg (PROTEINS L×d_h at drop=0.5; UNION anchor_boost; 40 select; gpu_h200 %6):
...
#   # → campaign sigma_grid_eval_proteins_reg_joint (30 eval; PROTEINS; gpu_h200 %6)
#
...
case "${phase}" in
    ...
    aggregate_sigma_native_fair_v2_joint)
        ...
        ;;
    aggregate_sigma_native_fair_v2_reddit_reg_joint)
        # UNION L12 + REDDIT lr×dropout refine; does not overwrite L12-only JSON.
        _run_python scripts/tu_errica/aggregate_sigma_hp_selection.py \
            --campaign sigma_grid_select_native_fair_v2 \
            --manifest configs/tu_errica/sigma_grids_native_fair_v2/manifest.json \
    ...
    sigma_grid_eval_native_fair_v2_reddit_reg_joint)
        TU_ERRICA_CAMPAIGN=sigma_grid_eval_native_fair_v2_reddit_reg_joint \
            TU_ERRICA_MEM=128GB TU_ERRICA_TIME=96:00:00 TU_ERRICA_NICE=0 \
    ...
    sigma_grid_eval_proteins_reg_joint)
        TU_ERRICA_CAMPAIGN=sigma_grid_eval_proteins_reg_joint \
            TU_ERRICA_MEM=128GB TU_ERRICA_TIME=72:00:00 TU_ERRICA_NICE=0 \
            TU_ERRICA_PARALLEL=6 TU_ERRICA_PARTITION=gpu_h200 \
