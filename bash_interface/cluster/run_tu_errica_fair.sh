...
case "${campaign}" in
    ...
    sigma_grid_select_proteins_reg)
        num_tasks=$(python3 -c "import json; print(json.load(open('configs/tu_errica/sigma_grids_proteins_reg/manifest.json'))['num_tasks'])")
        ;;
    ...
    sigma_grid_eval_anchor_boost|sigma_grid_eval_a1g2_micro)
        ...
        ;;
    sigma_grid_eval_a0g_pnr|sigma_grid_eval_tiny_pnr|sigma_grid_eval_specialist_tiny|sigma_grid_eval_native_fair|sigma_grid_eval_native_fair_v2|sigma_grid_eval_native_fair_v2_l4|sigma_grid_eval_native_fair_v2_joint)
        # PROTEINS + NCI1 + REDDIT (3 × 10 × 3).
        num_tasks=$((3 * num_folds * num_seeds))
        ;;
    sigma_grid_eval_a1g2_nci1_micro|sigma_grid_eval_anchor_refine|sigma_grid_eval_nci1_refine|sigma_grid_eval_native_fair_v2_reddit_reg_joint|sigma_grid_eval_proteins_reg_joint)
...
")
        ...
        ;;
    sigma_grid_select_proteins_reg)
        # PROTEINS L×d_h at drop=0.5 around anchor_boost mode; UNION later.
        cfg="configs/tu_errica/sigma-hetero-errica-base.yaml"
        model_key="sigma_hetero"
        model_tag="SiGMA_hetero"
        seed=$((seed_offset))
        read -r ds_tag fold_idx grid_rel hp_id < <(python3 -c "
import json
t=json.load(open('configs/tu_errica/sigma_grids_proteins_reg/manifest.json'))['tasks'][${idx}]
print(t['ds_tag'], t['fold'], t['grid_file'], t['hp_id'])
")
        sigma_grid_file="configs/tu_errica/sigma_grids_proteins_reg/grids/${grid_rel}"
        for i in "${!datasets[@]}"; do
            if [ "${datasets[$i]}" = "${ds_tag}" ]; then
                dataset_idx=$i
                break
...
")
    ...
    sigma_grid_eval_native_fair_v2_l4)
        # L=4-only winners; separate from L12-only and joint L∈{4,12}.
        cfg="configs/tu_errica/sigma-hetero-errica-base.yaml"
        model_key="sigma_hetero"
        model_tag="SiGMA_hetero"
        selection_file="${TU_ERRICA_SELECTION_FILE:-configs/tu_errica/selections/sigma_native_fair_v2_l4_per_fold.json}"
        seed=$((seed_offset + (idx % num_seeds)))
        rest=$((idx / num_seeds))
        fold_idx=$((rest % num_folds))
        local_ds=$((rest / num_folds))
        case "${local_ds}" in
            ...
            1) ds_tag="nci1"; dataset_idx=2 ;;
            2) ds_tag="reddit-b"; dataset_idx=5 ;;
            *) log_message "native_fair_v2_l4 eval local_ds=${local_ds} out of range"; exit 1 ;;
        esac
        use_selection=1
        ;;
    ...
    sigma_grid_eval_proteins_reg_joint)
        # UNION anchor_boost + proteins_reg; PROTEINS-only eval (10×3=30).
        cfg="configs/tu_errica/sigma-hetero-errica-base.yaml"
        model_key="sigma_hetero"
        model_tag="SiGMA_hetero"
        selection_file="${TU_ERRICA_SELECTION_FILE:-configs/tu_errica/selections/sigma_proteins_reg_joint_per_fold.json}"
        seed=$((seed_offset + (idx % num_seeds)))
        fold_idx=$((idx / num_seeds))
        if [ "${fold_idx}" -ge "${num_folds}" ]; then
            log_message "proteins_reg_joint eval fold=${fold_idx} out of range"
            exit 1
        fi
        ds_tag="proteins"
        dataset_idx=1
...
if [[ "${campaign}" != sigma_grid_select \
    ...
    && "${campaign}" != sigma_grid_select_proteins_reg \
