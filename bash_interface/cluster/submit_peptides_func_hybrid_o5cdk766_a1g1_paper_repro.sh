#!/usr/bin/env bash
# Submit peptides-func paper repro: o5cdk766 a1g1 (1 attn + 1× GCN) × 5 seeds.
#
# Usage:
#   source ~/.gnnplus_env
...
job_id="$(
    sbatch --parsable \
        --job-name=peptides_func_o5cdk766_a1g1 \
        --array="${ARRAY_SPEC}%${PARALLEL}" \
        --partition=mweber_gpu \
        --mem="${MEM}" \
        --time="${TIME}" \
        --gpus=1 \
        --output="logs_gnnplus/peptides_func_o5cdk766_a1g1_%A_%a.log" \
        --export=ALL,ENV_NAME=gnnplus,PAPER_NUM_SEEDS="${NUM_SEEDS}",GNNPLUS_DATASET_DIR="${GNNPLUS_DATASET_DIR:-}" \
        bash_interface/cluster/run_peptides_func_hybrid_o5cdk766_a1g1_paper_repro.sh
)"

echo ""
echo "=== peptides-func paper repro (o5cdk766 a1g1) submitted ==="
...
echo "  Source run:   https://wandb.ai/weber-geoml-harvard-university/GNNPlus/runs/o5cdk766"
echo "  Logs:         logs_gnnplus/peptides_func_o5cdk766_a1g1_${job_id}_<TASK>.log"
echo "  W&B group:    paper_bestmodel_v2_peptides_func_o5cdk766_a1g1_ep900"
...
echo "    --group paper_bestmodel_v2_peptides_func_o5cdk766_a1g1_ep900 \\"
