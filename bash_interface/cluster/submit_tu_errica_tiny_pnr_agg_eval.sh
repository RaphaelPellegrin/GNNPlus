#!/usr/bin/env bash
# Submit aggregate→eval chain for tiny_pnr (a2g4 bs×d_h on P/NCI1/REDDIT).
#
...
#
# Env: TU_ERRICA_DEPENDENCY_JOBID (optional), DEPENDENCY_TYPE, PARTITION, …
...
if [ -n "${DEPENDENCY_JOBID}" ]; then
  echo "[tiny_pnr agg_eval] dependency=${DEPENDENCY_TYPE}:${DEPENDENCY_JOBID}"
else
  echo "[tiny_pnr agg_eval] no dependency (run immediately)"
fi
echo "[tiny_pnr agg_eval] partition=${PARTITION} time=${TIME} mem=${MEM}"
echo "[tiny_pnr agg_eval] selection → configs/tu_errica/selections/sigma_tiny_pnr_per_fold.json"
echo "[tiny_pnr agg_eval] logs → ${LOGDIR}/agg_eval_<JOBID>.log"

if [ "${DRY_RUN}" = "1" ]; then
  echo "[tiny_pnr agg_eval] DRY_RUN=1 — not submitting"
  exit 0
fi

SBATCH_ARGS=(
  --parsable
  --job-name=tu_errica_tiny_pnr_agg
  --partition="${PARTITION}"
  --mem="${MEM}"
  --time="${TIME}"
  --nice="${NICE}"
...
JOBID="$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/run_tu_errica_tiny_pnr_agg_eval.sh")"

echo "Submitted tiny_pnr agg→eval JOBID=${JOBID}"
if [ -n "${DEPENDENCY_JOBID}" ]; then
