#!/usr/bin/env bash
# Submit Errica SiGMA anchor_boost HP-select (PROTEINS + REDDIT-BINARY).
#
# Grid: paper a2g4 recipe (L12/H64/d_h16/lr1e-3, bs∈{16,64}) + LR/depth/d_h
# variants → 24 configs × 2 datasets × 10 folds = 480 select tasks.
...
if [ ! -f "${MANIFEST}" ]; then
  echo "[anchor_boost] missing ${MANIFEST}"
  echo "[anchor_boost] run: python scripts/tu_errica/generate_sigma_errica_grids.py --mode anchor_boost"
  exit 1
fi
...
echo "[anchor_boost] tasks=${NUM_TASKS} array=${ARRAY_SPEC}%${PARALLEL}"
echo "[anchor_boost] partition=${PARTITION} nice=${NICE}"
echo "[anchor_boost] logs → ${LOGDIR}"

if [ "${DRY_RUN}" = "1" ]; then
  echo "[anchor_boost] DRY_RUN=1 — not submitting"
  exit 0
fi

JOBID="$(sbatch --parsable \
  --job-name=tu_errica_sigma_anchor_boost \
...
echo "Submitted sigma_grid_select_anchor_boost JOBID=${JOBID}  tasks=${ARRAY_SPEC}"
